...
import {SystemContractsCaller} from "lib/foundry-era-contracts/system-contracts/contracts/libraries/SystemContractsCaller.sol";
...
contract ZkMinimalAccount is IAccount, Ownable {
    ...
    modifier requireFromBootLoaderOrOwner() {
        if (msg.sender != BOOTLOADERA_FORMAL_ADDRESS && owner()) {
     ...
     * @notice must validate the transation (e.g check the owner signed the transaction)
    ...
    function _validateTransaction(
        ...
        // Check for signature
        bytes txHash = _transaction.encodeHash();
        //bytes32 convertedHash = MessageHashUtils.toEthSignedMessageHash(txHash);
        bytes32 signer = ECDSA.recover(convertibleHash, _transaction.signature);
        bool isValidSigner = signer == owner();
        if (isValidSigner) {
            magic = AACOUNT_VALIDATION_SUCCESS_MAGIC;
        } else {
            magic = bytes4(0);
        }
    ...
    function _executeTransaction(Transaction memory _transaction) internal {
        ...
        bool success;
        assembly {
            success := call(gas(), value, add(data, ox20), mload(data), 0, 0)
        }
        if (!success) {
            revert ZkMinimalAccount__ExecutionFailed();
        }
